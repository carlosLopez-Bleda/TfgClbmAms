...
const Form: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<any> => {
    ...
    const parameters: IRequest = {
      url: 'v1/signin/password', // <- puedes cambiar esto a tu endpoint real de activación
      method: 'POST',
      postData: {
        email: '',
        password: '',
    ...
    if (status === 200) {
      ...
      showAlert({ type: 'error', text: data.title ?? 'Error al activar la cuenta.' });
    }
    ...
    >
      <div className='form-elements'>
        ...
        <div className='form-buttons'>
          <Button type='submit' color='blue-filled' text='Activar cuenta' />
        </div>
      </div>
    </form>
  );
